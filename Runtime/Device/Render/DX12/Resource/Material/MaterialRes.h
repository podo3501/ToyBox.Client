...
class MaterialRes : public IMaterialResource //backend용 material resource 인터페이스.
{
public:
	virtual ~MaterialRes() override;
	MaterialRes() = delete;
	MaterialRes(uint32_t texSlotCount);
	...
	TextureResource* GetTextureRaw(TextureSlot texSlot) const noexcept;
	std::shared_ptr<TextureResource> GetTexture(TextureSlot texSlot) const noexcept;
