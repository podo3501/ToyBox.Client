...
//SDF는 하나의 크기만 찍고 그걸 줄이거나 늘려서 사용한다. 그래서 페이지 절약이 된다.
//20폰트 이하는 SDF를 쓸 필요가 없다면(아웃라인이나 줌아웃 같은게 필요 없다면) bitmap이 실용적이다.
//실제로는 bitmap과 SDF도 작은 폰트에도 그렇게 차이가 없기 때문에 일괄적으로 SDF 로 해도 되긴 한다. 테스트 해 보고 쓰는 사람이 결정하도록 한다.
...
namespace SDFBuckets
{
    ...
    constexpr FontBucketID Medium = 50;
    constexpr FontBucketID FontMaxSize = 100;
}

namespace MTSDFBuckets
{
    constexpr FontBucketID Small = 20;
    constexpr FontBucketID Medium = 40;
}
...
inline FontBucketID GetFontBucketID(TextRenderMode mode, uint32_t size)
{
    ...
    case TextRenderMode::SDF:
        ...
        if (size <= SDFBuckets::FontMaxSize) return SDFBuckets::Medium;
        break;
...
inline uint32_t GetSDFResolution(uint32_t size)
{
    return static_cast<uint32_t>(GetFontBucketID(TextRenderMode::SDF, size));
}
